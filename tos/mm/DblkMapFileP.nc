 ...
 * (2a) Call cache_copy(src=508, dst=0, len=4) to copy the
 ...
 * (3)  Call sd.read(blk_id, &cache[cache.len], 512) to start
 *      retrieving the next 512 bytes of data. It will be stored
 *      immediately after prepend data in cache (partial data
 *      from previous sector)
 * (3a) DMF.mapAll call returns <= (EBUSY) to indiate that it
 *      is busy handling the callers request
 * (4)  Time passes as disk read is performed
 * (5)  Call to sd.readDone(sector=blk_id, buf=&cache[cache.len],
 *      len=512) verifies and updates state information based
 *      on success of the sector read.
 * (5a) Cache state after SD.readDone() success shows that the
 ...
 * (5b) DMF.data_avail() is called to signal that new data is
 *      available for user
 * (6)  Time passes as user task is scheduled
 * (7)  User calls DMF.mapAll(offset=508, len=8) again to
 *      see if data is now available.
 * (7a) Returns <= (SUCCESS, data=[1234abcd], len=8) succcess
...
typedef struct {
  struct {
    ...
    uint32_t             id;         // physical blk number, 0 = cache empty
  } cache;
...
implementation {
  ...
  error_t mapit(uint32_t context, uint8_t **bufp,
     ...
     *   - if MAP_ALL
     *     - offset + size is less than end of cache (cache.offset + cache.len)
     *   - if MAP_ANY
    ...
    if (dmf_cb.cache.id &&
      ...
      *bufp = &dmf_cache[(offset - dmf_cb.cache.offset)];
      ...
      if (len_avail < *lenp) {
        if (map_mode) // shouldn't get here because of conditional tests above
          dmap_panic(2, len_avail, *lenp);
        *lenp = len_avail;
      }
      return SUCCESS;
    }
    ...
    /*
     * Cache Miss, first need to preserve any cached data needed to
     * satisfy MAP_ALL mode request with data spanning sector boundary.
    ...
    if (dmf_cb.cache.id &&
      ...
      dmf_cb.cache.len = copy_block(
        ...
        dmf_cb.cache.offset + dmf_cb.cache.len - RNDWORDDN(offset));
      dmf_cb.cache.offset = RNDBLKUP(offset);
      offset = RNDWORDDN(offset) + dmf_cb.cache.len;
      if (offset % SD_BLOCKSIZE)
        dmap_panic(11, offset, dmf_cb.cache.len);
    } else {
      dmf_cb.cache.offset = RNDBLKDN(offset);
      dmf_cb.cache.len = 0;
    }
    dmf_cb.cache.id = 0;     /* invalidate current cache */
    ...
    if (blk_buf) {
      if (blk_offset != (dmf_cb.cache.offset + dmf_cb.cache.len))
        dmap_panic(3, blk_offset, (dmf_cb.cache.offset + dmf_cb.cache.len));
      ...
      *bufp = &dmf_cache[(offset - dmf_cb.cache.offset)];
      ...
      len_avail = dmf_cb.cache.offset + dmf_cb.cache.len - offset;
      if (len_avail < *lenp){
        if (map_mode) // shouldn't get here
          dmap_panic(4, len_avail, *lenp);
        *lenp = len_avail;
      }
      return SUCCESS;
    }
  ...
  event void SDread.readDone(uint32_t blk_id, uint8_t *read_buf, error_t err) {
    ...
    dmf_cb.requested  = FALSE;
    dmf_cb.reading    = FALSE;
    dmf_cb.cache.id   = blk_id;        /* valid cache now */
